...
public class ProductController : ControllerBase
{
    ...
    public async Task<IActionResult> GetAllAsync()
    {
        try
        {
            IEnumerable<Product> products = await _service.GetAllAsync(HttpContext.RequestAborted);
    ...
    public async Task<IActionResult> GetImageAsync([FromQuery] [Required] Guid productId)
    {
        try
        {
            ...
            if (imageStream != null)
            {
                string? mimeType = _inspector.Inspect(imageStream).First().Definition.File.MimeType;
